...
use App\Http\Requests\warehouse\ReturnFinishStockRequest;
...
use App\Models\ReturnFinishStock;
use App\Models\ReturnFinishStockItem;
use App\Models\User;
...
class ReturnFinishStockController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $returnFinishStock = ReturnFinishStock::with('company')
        ...
        $tabArr = [
            ['label' => 'All', 'value' => 'all', 'count' => $returnFinishStock->count(), 'color' => 'primary'],
            ['label' => 'Pending', 'value' => 'pending', 'count' => $returnFinishStock->where('status', 'pending')->count(), 'color' => 'warning'],
            ['label' => 'Approved', 'value' => 'approved', 'count' => $returnFinishStock->where('status', 'approved')->count(), 'color' => 'success'],
            ['label' => 'Rejected', 'value' => 'reject', 'count' => $returnFinishStock->where('status', 'reject')->count(), 'color' => 'danger'],
        ];
        $tabs = statusTabs($tabArr, $status);
        if ($request->ajax()) {
            $data = $returnFinishStock;
            if ($status != 'all')
        ...
        return view('admin.warehouse.returnfinishstock.index', compact('tabs', 'status', 'fromDate', 'toDate'));
    }
    public function create()
    {
        ...
        $returnFinishStock = new ReturnFinishStock();
        $returnFinishStock_code = $returnFinishStock::pluck('code')->toArray();
        $returnFinishStock->code = generateUniqueCode('REFST-', $returnFinishStock_code);
        return view('admin.warehouse.returnfinishstock.create', compact('returnFinishStock', 'companies', 'branch', 'status'));
    }
    public function store(ReturnFinishStockRequest $request)
    {
        ...
        $returnFinishStock = ReturnFinishStock::create([
            ...
            'crated_at' => $request->has('crated_at') ? $request->crated_at : date('Y-m-d H:i:s'),
            'created_by' => $data['user_creator_id'],
        ]);
        $this->setItems($request->items, $returnFinishStock->id, $returnFinishStock->company_id);
        Session::flash('success', 'Created successfully.');
        $redirect = route('returnfinishstock.index');
    ...
    public function edit(string $id)
    {
        ...
        $returnFinishStock = ReturnFinishStock::find($id);
        $items = ReturnFinishStockItem::where('issue_requisition_id', $id)->get()->toArray();
        $companies = Company::where('id', $this->company_id)->get();
        $branch = Branch::where('company_id', $this->company_id)->get();
        return view('admin.warehouse.returnfinishstock.create', compact('returnFinishStock', 'items', 'companies', 'branch', 'status'));
    }
    public function update(Request $request, string $id)
    {
        $data = $request->except('_token', '_method', 'items', 'summary');
        $data = $this->summary($request->summary, $data);
        ReturnFinishStock::find($request->id)->update([
            'code' => $data['code'],
            'title' => $data['title'],
            'currency_code' => $data['currency_code'],
            'tax_type' => $data['tax_type'],
    ...
    public function toggleActive(string $id)
    {
        ...
        $returnFinishStock = ReturnFinishStock::find($id);
        if ($returnFinishStock->is_active == 1) {
            $returnFinishStock->is_active = 0;
        }
        if ($returnFinishStock->is_active == 0) {
            $returnFinishStock->is_active = 1;
        }
        $returnFinishStock->save();
    ...
    private function setItems($items, $id, $company_id)
    {
        ...
        $oldItemCode = [];
        $oldItems = ReturnFinishStockItem::where('return_finish_stock_id', $id)->get();
        if (count($oldItems) > 0) {
            $codes = [];
        ...
        if (count($items)) {
            foreach ($items as $item) {
                ...
                if (in_array($item->code, $oldItemCode)) {
                    ReturnFinishStockItem::where([['return_finish_stock_id', $id], ['code', $item->code]])->update((array) $item);
                } else {
                    ReturnFinishStockItem::create([
                        'return_finish_stock_id' => $id,
