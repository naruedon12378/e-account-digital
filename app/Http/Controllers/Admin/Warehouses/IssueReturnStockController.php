...
use App\Http\Requests\warehouse\IssueReturnStockRequest;
use App\Models\Branch;
use App\Models\Company;
use App\Models\CompanyAddress;
use App\Models\Inventory;
use App\Models\InventoryStock;
use App\Models\IssueReturnStock;
use App\Models\IssueReturnStockItem;
use App\Models\User;
...
class IssueReturnStockController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $issueReturnStocks = IssueReturnStock::with('company')
            ->where('is_active', 1)
        ...
        $tabArr = [
            ['label' => 'All', 'value' => 'all', 'count' => $issueReturnStocks->count(), 'color' => 'primary'],
            ['label' => 'Pending', 'value' => 'pending', 'count' => $issueReturnStocks->where('status', 'pending')->count(), 'color' => 'warning'],
            ['label' => 'Approved', 'value' => 'approved', 'count' => $issueReturnStocks->where('status', 'approved')->count(), 'color' => 'success'],
            ['label' => 'Rejected', 'value' => 'reject', 'count' => $issueReturnStocks->where('status', 'reject')->count(), 'color' => 'danger'],
        ];
        $tabs = statusTabs($tabArr, $status);
        if ($request->ajax()) {
            $data = $issueReturnStocks;
            if ($status != 'all')
        ...
        return view('admin.warehouse.issuereturnstock.index', compact('tabs', 'status', 'fromDate', 'toDate'));
    }
    ...
    public function create()
    {
        ...
        $issueReturnStock = new IssueReturnStock();
        $issueReturnStock_code = $issueReturnStock::pluck('code')->toArray();
        $issueReturnStock->code = generateUniqueCode('IREST-',$issueReturnStock_code);
        return view('admin.warehouse.issuereturnstock.create', compact('issueReturnStock', 'companies', 'branch', 'status'));
    }

    public function store(IssueReturnStockRequest $request)
    {
        ...
        $issueReturnStock = IssueReturnStock::create([
        ...
        $this->setItems($request->items, $issueReturnStock->id, $issueReturnStock->company_id);
        Session::flash('success', 'Created successfully.');
        $redirect = route('issuereturnstock.index');
    ...
    public function edit(string $id)
    {
        ...
        $issueReturnStock = IssueReturnStock::find($id);
        $items = IssueReturnStockItem::where('issue_return_stock_id', $id)->get()->toArray();
        $companies = Company::where('id', $this->company_id)->get();
        $branch = Branch::where('company_id', $this->company_id)->get();
        return view('admin.warehouse.issuereturnstock.create', compact('issueReturnStock', 'items', 'companies', 'branch', 'status'));
    }
    public function update(Request $request, string $id)
    {
        $data = $request->except('_token', '_method', 'items', 'summary');
        $data = $this->summary($request->summary, $data);
        IssueReturnStock::find($request->id)->update([
            'code' => $data['code'],
            'title' => $data['title'],
            'branch_id' => $data['branch_id'],
            'remark' => $data['remark'],
    ...
    public function destroy(string $id)
    {
        ...
        IssueReturnStock::whereId($id)->update([
            'is_active' => 0,
    ...
    public function toggleActive(Request $request, string $id)
    {
        ...
        $issueReturnStock = IssueReturnStock::find($id);
        if ($issueReturnStock->is_active == 1) {
            $issueReturnStock->is_active = 0;
        }
        if ($issueReturnStock->is_active == 0) {
            $issueReturnStock->is_active = 1;
        }
        $issueReturnStock->save();
    ...
    private function setItems($items, $id, $company_id)
    {
        ...
        $oldItemCode = [];
        $oldItems = IssueReturnStockItem::where('issue_return_stock_id', $id)->get();
        if (count($oldItems) > 0) {
            $codes = [];
        ...
        if (count($items)) {
            foreach ($items as $item) {
                ...
                if (in_array($item->code, $oldItemCode)) {
                    IssueReturnStockItem::where([['issue_return_stock_id', $id], ['code', $item->code]])->update((array) $item);
                } else {
                    IssueReturnStockItem::create([
                        'issue_return_stock_id' => $id,
