...
use App\Http\Requests\warehouse\TransferRequistionRequest;
...
use App\Models\TransferRequistion;
use App\Models\TransferRequistionItem;
use App\Models\User;
...
class TransferRequistionController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $transferRequistions = TransferRequistion::with('company')
        ...
        $tabArr = [
            ['label' => 'All', 'value' => 'all', 'count' => $transferRequistions->count(), 'color' => 'primary'],
            ['label' => 'Pending', 'value' => 'pending', 'count' => $transferRequistions->where('status', 'pending')->count(), 'color' => 'warning'],
            ['label' => 'Approved', 'value' => 'approved', 'count' => $transferRequistions->where('status', 'approved')->count(), 'color' => 'success'],
            ['label' => 'Rejected', 'value' => 'reject', 'count' => $transferRequistions->where('status', 'reject')->count(), 'color' => 'danger'],
        ];
        $tabs = statusTabs($tabArr, $status);
        if ($request->ajax()) {
            $data = $transferRequistions;
            if ($status != 'all')
        ...
        return view('admin.warehouse.transferrequistion.index', compact('tabs', 'status', 'fromDate', 'toDate'));
    }
    public function create()
    {
        ...
        $transferRequistion = new TransferRequistion();
        $transferRequistion_code = $transferRequistion::pluck('code')->toArray();
        $transferRequistion->code = generateUniqueCode('ISR-', $transferRequistion_code);
        return view('admin.warehouse.transferrequistion.create', compact('transferRequistion', 'companies', 'branch', 'status'));
    }
    public function store(TransferRequistionRequest $request)
    {
        ...
        $transferRequistion = TransferRequistion::create([
        ...
        $this->setItems($request->items, $transferRequistion->id, $transferRequistion->company_id);
        Session::flash('success', 'Created successfully.');
        $redirect = route('transferrequistion.index');
    ...
    public function edit(string $id)
    {
        ...
        $transferRequistion = TransferRequistion::find($id);
        $items = TransferRequistionItem::where('transfer_requistion_id', $id)->get()->toArray();
        $companies = Company::where('id', $this->company_id)->get();
        $branch = Branch::where('company_id', $this->company_id)->get();
        return view('admin.warehouse.transferrequistion.create', compact('transferRequistion', 'items', 'companies', 'branch', 'status'));
    }
    public function update(Request $request, string $id)
    {
        $data = $request->except('_token', '_method', 'items', 'summary');
        $data = $this->summary($request->summary, $data);
        TransferRequistion::find($request->id)->update([
            'code' => $data['code'],
            'title' => $data['title'],
            'branch_id' => $data['branch_id'],
            'remark' => $data['remark'],
    ...
    public function toggleActive(string $id)
    {
        ...
        $transferRequistion = TransferRequistion::find($id);
        if ($transferRequistion->is_active == 1) {
            $transferRequistion->is_active = 0;
        }
        if ($transferRequistion->is_active == 0) {
            $transferRequistion->is_active = 1;
        }
        $transferRequistion->save();
    ...
    private function summary($summary, $data)
    {
        ...
        $data['title'] = 'Transfer Requisition For' . $company_name;
    ...
    private function setItems($items, $id, $company_id)
    {
        ...
        $oldItemCode = [];
        $oldItems = TransferRequistionItem::where('transfer_requistion_id', $id)->get();
        if (count($oldItems) > 0) {
            $codes = [];
        ...
        if (count($items)) {
            foreach ($items as $item) {
                ...
                if (in_array($item->code, $oldItemCode)) {
                    TransferRequistionItem::where([['transfer_requistion_id', $id], ['code', $item->code]])->update((array) $item);
                } else {
                    TransferRequistionItem::create([
                        'transfer_requistion_id' => $id,
