...
use App\Http\Requests\warehouse\ReceiveFinishStockRequest;
...
use App\Models\ReceiveFinishStock;
use App\Models\ReceiveFinishStockItem;
use App\Models\User;
...
class ReceiveFinishStockController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $receiveFinishStock = ReceiveFinishStock::with('company')
        ...
        $tabArr = [
            ['label' => 'All', 'value' => 'all', 'count' => $receiveFinishStock->count(), 'color' => 'primary'],
            ['label' => 'Pending', 'value' => 'pending', 'count' => $receiveFinishStock->where('status', 'pending')->count(), 'color' => 'warning'],
            ['label' => 'Approved', 'value' => 'approved', 'count' => $receiveFinishStock->where('status', 'approved')->count(), 'color' => 'success'],
            ['label' => 'Rejected', 'value' => 'reject', 'count' => $receiveFinishStock->where('status', 'reject')->count(), 'color' => 'danger'],
        ];
        $tabs = statusTabs($tabArr, $status);
        if ($request->ajax()) {
            $data = $receiveFinishStock;
            if ($status != 'all')
        ...
        return view('admin.warehouse.receivefinishstock.index', compact('tabs', 'status', 'fromDate', 'toDate'));
    }
    public function create()
    {
        ...
        $receiptPlannings = ReceiptPlanning::where('company_id', $this->company_id)->get();
        $branch = Branch::where('company_id', $this->company_id)->get();
        $receiveFinishStock = new ReceiveFinishStock();
        $receiveFinishStock_code = $receiveFinishStock::pluck('code')->toArray();
        $receiveFinishStock->code = generateUniqueCode('RECFIST-',$receiveFinishStock_code);
        return view('admin.warehouse.receivefinishstock.create', compact('receiveFinishStock', 'companies','receiptPlannings', 'branch', 'status'));
    }
    public function store(ReceiveFinishStockRequest $request)
    {
        // dd($request->all());
        $validator = Validator::make($request->all(), [
            'doc_number' => 'unique:invoices',
            'company_id' => 'required',
            'receipt_planning_id' => 'required',
        ]);
        if ($validator->fails()) {
            return response()->json(['errors' => $validator->errors()], 409);
        }
        if($request->receipt_planning_id){
            $receiptPlanning = ReceiptPlanning::findOrFail($request->receipt_planning_id);
            $receipt_document_code = $receiptPlanning->code;
            $receipt_planning_id = $request->receipt_planning_id;
        }else{
            $data['receipt_planning_id'] = 1;
            $data['receipt_document_code'] = 'SYSTEM-RECEIPT-PLANNING';
        }
        $data = $request->except('_token', 'items', 'summary');
        // $summary = (array) json_decode($request->summary);
        $data = $this->summary($request->summary, $data);
        $receiveFinishStock = ReceiveFinishStock::create([
            'company_id' => $data['company_id'],
            'code' => $data['code'],
            'receipt_planning_id' => $receipt_planning_id,
            'receipt_document_code' => $receipt_document_code,
            'currency_code' => $data['currency_code'],
            'tax_type' => $data['tax_type'],
            'title' => $data['title'],
            'branch_id' => $data['branch_id'],
            'created_date' => $request->create_at,
            'remark' => $data['remark'],
            'status' => $data['status'],
        ...
        $this->setItems($request->items, $receiveFinishStock->id, $receiveFinishStock->company_id);
        Session::flash('success', 'Created successfully.');
        $redirect = route('receivefinishstock.index');
    ...
    public function edit(string $id)
    {
        ...
        $receiveFinishStock = ReceiveFinishStock::find($id);
        $items = ReceiveFinishStockItem::where('receive_finish_stock_id', $id)->get()->toArray();
        $companies = Company::where('id', $this->company_id)->get();
        $receiptPlannings = ReceiptPlanning::where('company_id', $this->company_id)->get();
        $branch = Branch::where('company_id', $this->company_id)->get();
        return view('admin.warehouse.receivefinishstock.create', compact('receiveFinishStock', 'items', 'companies','receiptPlannings', 'branch', 'status'));
    }
    public function update(Request $request, string $id)
    {
        $data = $request->except('_token', '_method', 'items', 'summary');
        $data = $this->summary($request->summary, $data);
        ReceiveFinishStock::find($request->id)->update([
            'code' => $data['code'],
            'title' => $data['title'],
            'currency_code' => $data['currency_code'],
            'tax_type' => $data['tax_type'],
    ...
    public function toggleActive(Request $request, string $id)
    {
        ...
        $receiveFinishStock = ReceiveFinishStock::find($id);
        if ($receiveFinishStock->is_active == 1) {
            $receiveFinishStock->is_active = 0;
        }
        if ($receiveFinishStock->is_active == 0) {
            $receiveFinishStock->is_active = 1;
        }
        $receiveFinishStock->save();
    ...
    private function summary($summary, $data)
    {
        ...
        $data['title'] = 'Receive Finish Stock For' . $company_name;
    ...
    private function setItems($items, $id, $company_id)
    {
        ...
        $oldItemCode = [];
        $oldItems = ReceiveFinishStockItem::where('receive_finish_stock_id', $id)->get();
        if (count($oldItems) > 0) {
            $codes = [];
        ...
        if (count($items)) {
            foreach ($items as $item) {
                ...
                $item->receive_finish_stock_id = $id;
                ...
                if (in_array($item->code, $oldItemCode)) {
                    ReceiveFinishStockItem::where([['receive_finish_stock_id', $id], ['code', $item->code]])->update((array) $item);
                } else {
                    ReceiveFinishStockItem::create([
                        'receive_finish_stock_id' => $id,
