...
function rowItemTable(product) {
    ...
    if (flag) {
        ...
        var cols = "";
        cols += `<td class="text-center pZero line_item_no"></td>`;
        ...
        cols += `<td class="text-right pZero">
                    <input
                        type="number"
        ...
        cols += `<td class="text-right price">${product.cost_price}</td>`;
        ...
        cols += `<td class="text-center pZero">
...
function setStateItem(product) {
    ...
    state.item.product_id = product.product_id;
    state.item.account_code = product.account_code;
    state.item.price = product.cost_price;
...
function updateRowItem(index) {
    ...
    rowPrice.text(state.item.price);
    rowDiscount.text(state.item.discount);
    rowPreVatAmt.text(pre_vat_amt);
...
function editRowItem(code) {
    ...
    if (filter.length > 0) {
        ...
        state.item.product_id = filter[0].product_id;
