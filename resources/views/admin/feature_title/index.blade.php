...
@php $pagename = 'จัดการหัวข้อฟีเจอร์'; @endphp
...
@section('content')
    <div class="pt-3">
        ...
        <div class="card {{ env('CARD_THEME') }} shadow-custom">
            ...
            <div class="card-body">
                ...
                @if (Auth::user()->hasAnyPermission(['*', 'all feature_title', 'create feature_title']))
                ...
                <table id="table" class="table table-hover dataTable no-footer nowrap" style="width: 100%;">
                    <thead class="bg-custom">
                        <tr>
                            ...
                            <th class="text-center">{{ __('feature.feature_title_th') }}</th>
                            <th class="text-center">{{ __('feature.feature_title_en') }}</th>
    ...
    @push('js')
        <script>
            ...
            function requestData(response) {
                $('#eid').val(response.feature_title.id);
                $('#ename_th').val(response.feature_title.name_th);
                $('#ename_en').val(response.feature_title.name_en);
            ...
            function updateData() {
                ...
                id = $('#eid').val();
                // url = '{{ route('feature_title.update', ['feature_title' => ':id']) }}';
                // url = url.replace(':id', id);
                url = '{{ route('feature_title.update') }}';
    ...
    @include('admin.feature_title.partials.create-modal')
    @include('admin.feature_title.partials.edit-modal')
@endsection
