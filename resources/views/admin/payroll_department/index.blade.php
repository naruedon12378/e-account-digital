...
@section('content')
    <div class="pt-3">
        ...
        <div class="card {{ env('CARD_THEME') }} shadow-custom">
            ...
            <div class="card-body">
                ...
                @if (Auth::user()->hasAnyPermission(['*', 'all payroll_department', 'create payroll_department']))
                ...
                <table id="table" class="table table-hover dataTable no-footer nowrap" style="width: 100%;">
                    <thead class="bg-custom">
                        <tr>
                            <th class="text-center" style="width: 10%">{{ __('payroll_department.number') }}</th>
                            <th class="text-center">{{ __('payroll_department.department_name_th') }}</th>
                            <th class="text-center">{{ __('payroll_department.department_name_en') }}</th>
                            <th class="text-center" style="width: 10%">
                                {{ __('payroll_department.adjust_salary_to_account_code') }}</th>
                            <th class="text-center" style="width: 10%">{{ __('payroll_department.status') }}</th>
                            <th class="text-center" style="width: 10%">{{ __('payroll_department.actions') }}</th>
                        </tr>
                    </thead>
                    <tbody>
                    </tbody>
    ...
    @push('js')
        <script>
            ...
            function storeData(url) {
                ...
                url = "{{ route('payroll_department.store') }}";
            ...
            function requestData(response) {
                $('#eid').val(response.payroll_department.id);
                $('#ename_th').val(response.payroll_department.name_th);
                $('#ename_en').val(response.payroll_department.name_en);
            ...
            function updateData() {
                ...
                id = $('#eid').val();
                // url = '{{ route('payroll_department.update', ['payroll_department' => ':id']) }}';
                // url = url.replace(':id', id);
                url = '{{ route('payroll_department.update') }}';
                ...
                submitData(url, eformdata).then(function(response) {
                    $('#close-modal').click();
                });
            }
        </script>
    ...
    @include('admin.payroll_department.partials.create-modal')
    @include('admin.payroll_department.partials.edit-modal')
@endsection
