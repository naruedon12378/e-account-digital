...
@php $pagename = 'จัดการฟีเจอร์'; @endphp
...
@section('content')
    <div class="pt-3">
        ...
        <div class="card {{ env('CARD_THEME') }} shadow-custom">
            ...
            <div class="card-body">
                ...
                @if (Auth::user()->hasAnyPermission(['*', 'all feature', 'create feature']))
                ...
                <table id="table" class="table table-hover dataTable no-footer nowrap" style="width: 100%;">
                    <thead class="bg-custom">
                        <tr>
                            <th class="text-center" style="width: 10%">{{ __('feature.number') }}</th>
                            <th class="text-center">{{ __('feature.feature_th') }}</th>
                            <th class="text-center">{{ __('feature.feature_en') }}</th>
                            <th class="text-center">{{ __('feature.feature_title') }}</th>
                            <th class="text-center" style="width: 10%">{{ __('feature.status') }}</th>
                            <th class="text-center" style="width: 10%">{{ __('feature.sort') }}</th>
                            <th class="text-center" style="width: 10%">{{ __('feature.actions') }}</th>
                        </tr>
                    </thead>
                    <tbody>
                    </tbody>
    ...
    @push('js')
        <script>
            ...
            function storeData(url) {
                ...
                formdata.append('feature_title', $('#feature_title').val());
            ...
            function requestData(response) {
                $('#eid').val(response.feature.id);
                $('#ename_th').val(response.feature.name_th);
                $('#ename_en').val(response.feature.name_en);
                $('#efeature_title').val(response.feature.feature_title_id);
                $('#efeature_title').select2();
            ...
            function updateData() {
                ...
                eformdata.append('feature_title', $('#efeature_title').val());
    ...
    @include('admin.feature.partials.create-modal')
    @include('admin.feature.partials.edit-modal')
@endsection
