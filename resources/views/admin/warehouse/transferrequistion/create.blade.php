...
@php($pagename = 'Transfer Requisition')
...
@section('content')
    ...
    <x-form-card>
        <x-form name="frmPurchase"
            :action="$transferRequistion->id ? route('transferrequistion.update', $transferRequistion->id) : route('transferrequistion.store')" enctype>
            @if ($transferRequistion->id)
                @method('PUT')
                <input type="hidden" name="id" value="{{ $transferRequistion->id }}">
            @endif
            <div class="border-bottom border-success px-3 pt-3">
                <div class="d-flex justify-content-between flex-wrap">
                    <h4>Create Transfer Requisition</h4>
                    <x-form-group label="beginning_balance.code" name="code" required
                        value="{{ $transferRequistion->code }}">
                    </x-form-group>
                </div>
            </div>
            <x-warehouse.collapse number="1" title="Seller" show>
                <div class="row">
                    <div class="col-12 col-md-6">
                        @if($transferRequistion && $transferRequistion->id && $transferRequistion->company_id)
                            <x-warehouse.form-select label="{{ __('beginning_balance.company') }}" :selectoption="'beginning_balance.select_company'"
                                name="company_id" class="select2" required :value="$companies" :selectedvalue="$transferRequistion->company_id"
                    ...
                    <div class="col-12 col-md-3">
                        <x-datepicker label="beginning_balance.created_at" name="created_at" :value="$transferRequistion->created_at" required>
                        </x-datepicker>
                    </div>
                </div>
            ...
            <x-warehouse.collapse number="3" title="Pricing and tax setting" show>
                <div class="row">
                    <div class="col-12 col-md-4">
                        @if($transferRequistion && $transferRequistion->id)
                            <x-form-select label="beginning_balance.transfer-requisition_status" name="status"  class="select2" required
                                :value="$transferRequistion->status" :data="$status"></x-form-select>
                        @else
                            <x-warehouse.status-form-select label="beginning_balance.transfer-requisition_status" name="status" :selectoption="'beginning_balance.please_select_status'" :selectedvalued="'pending'" class="select2" required
                    ...
                    <div class="col-12 col-md-4">
                        <x-form-select label="beginning_balance.tax_type" name="tax_type" class="select2" required
                            :value="$transferRequistion->status" :data="pricingTypes()"></x-form-select>
                    </div>
                    <div class="col-12 col-md-4">
                        <x-form-select label="beginning_balance.currency_code" name="currency_code" class="select2" required
                            :value="$transferRequistion->currency_code" :data="currencies()"></x-form-select>
                    </div>
                </div>
            </x-warehouse.collapse>
            <x-warehouse.collapse number="4" title="{{ __('beginning_balance.transfer-requisition-information')}}" show>
            ...
            <x-warehouse.collapse number="6" title="Remark for customer" show>
                <x-textarea label="beginning_balance.remark" name="remark" :value="$transferRequistion->remark"></x-textarea>
            </x-warehouse.collapse>
            <div class="text-center mt-5">
                <x-button-cancel url="{{ route('transferrequistion.index') }}">
                    {{ __('file.Back') }}
                </x-button-cancel>
