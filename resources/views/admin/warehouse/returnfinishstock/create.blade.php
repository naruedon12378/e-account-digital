...
@php($pagename = 'Return Finish Stock')
...
@section('content')
    ...
    <x-form-card>
        <x-form name="frmPurchase"
            :action="$returnFinishStock->id ? route('returnfinishstock.update', $returnFinishStock->id) : route('returnfinishstock.store')" enctype>
            @if ($returnFinishStock->id)
                @method('PUT')
                <input type="hidden" name="id" value="{{ $returnFinishStock->id }}">
            @endif
            <div class="border-bottom border-success px-3 pt-3">
                <div class="d-flex justify-content-between flex-wrap">
                    <h4>Create Return Finish Stock</h4>
                    <x-form-group label="beginning_balance.code" name="code" required
                        value="{{ $returnFinishStock->code }}">
                    </x-form-group>
                </div>
            </div>
            <x-warehouse.collapse number="1" title="Seller" show>
                <div class="row">
                    <div class="col-12 col-md-6">
                        @if($returnFinishStock && $returnFinishStock->id && $returnFinishStock->company_id)
                            <x-warehouse.form-select label="{{ __('beginning_balance.company') }}" :selectoption="'beginning_balance.select_company'"
                                name="company_id" class="select2" required :value="$companies" :selectedvalue="$returnFinishStock->company_id"
                    ...
                    <div class="col-12 col-md-3">
                        <x-datepicker label="beginning_balance.created_at" name="created_at" :value="$returnFinishStock->created_at" required>
                        </x-datepicker>
                    </div>
                </div>
            ...
            <x-warehouse.collapse number="3" title="Pricing and tax setting" show>
                <div class="row">
                    <div class="col-12 col-md-4">
                        @if($returnFinishStock && $returnFinishStock->id)
                            <x-form-select label="beginning_balance.return-finish-stock_status" name="status"  class="select2" required
                                :value="$returnFinishStock->status" :data="$status"></x-form-select>
                        @else
                            <x-warehouse.status-form-select label="beginning_balance.return-finish-stock_status" name="status" :selectoption="'beginning_balance.please_select_status'" :selectedvalued="'pending'" class="select2" required
                    ...
                    <div class="col-12 col-md-4">
                        <x-form-select label="beginning_balance.tax_type" name="tax_type" class="select2" required
                            :value="$returnFinishStock->status" :data="pricingTypes()"></x-form-select>
                    </div>
                    <div class="col-12 col-md-4">
                        <x-form-select label="beginning_balance.currency_code" name="currency_code" class="select2" required
                            :value="$returnFinishStock->currency_code" :data="currencies()"></x-form-select>
                    </div>
                </div>
            </x-warehouse.collapse>
            ...
            <x-warehouse.collapse number="6" title="Remark for customer" show>
                <x-textarea label="beginning_balance.remark" name="remark" :value="$returnFinishStock->remark"></x-textarea>
            </x-warehouse.collapse>
            <div class="text-center mt-5">
                <x-button-cancel url="{{ route('returnfinishstock.index') }}">
                    {{ __('file.Back') }}
                </x-button-cancel>
